CREATE OR REPLACE FUNCTION fr_find_recently_closed() RETURNS void AS $$
DECLARE
  weekrow record;
BEGIN
    ...
    LOOP
        ...
        INSERT INTO fr_recently_closed (
            SELECT date,
...
UPDATE fr_task_history
   SET status = '"open"'
 WHERE status = '"stalled"';

DELETE FROM fr_task_history
...
) to '/tmp/fr_backlog.csv' DELIMITER ',' CSV HEADER;
...
SELECT date,
 ...
 GROUP BY date, category
 ORDER BY date, category
) to '/tmp/fr_backlog_count.csv' DELIMITER ',' CSV HEADER;
...
SELECT date,
 ...
 GROUP BY date
 ORDER BY date
) to '/tmp/fr_burnup_count.csv' DELIMITER ',' CSV HEADER;
...
SELECT date,
 ...
 GROUP BY date, category
 ORDER BY date, category
) to '/tmp/fr_burnup_categories.csv' DELIMITER ',' CSV HEADER;
...
SELECT date,
 ...
 GROUP BY date, category
 ORDER BY date, category
) to '/tmp/fr_burnup_categories_count.csv' DELIMITER ',' CSV HEADER;

/* ####################################################################
Burnup and Velocity */

DROP TABLE IF EXISTS fr_velocity_week;
DROP TABLE IF EXISTS fr_velocity_delta;

SELECT date,
       SUM(points) AS points
  INTO fr_velocity_week
  FROM fr_tall_backlog
 WHERE status = '"resolved"'
...
SELECT date,
  ...
  INTO fr_velocity_delta
  FROM fr_velocity_week
 ORDER BY date;

UPDATE fr_velocity_delta a
...
SELECT date,
       velocity
  FROM fr_velocity_delta
) TO '/tmp/fr_velocity.csv' DELIMITER ',' CSV HEADER;

DROP TABLE IF EXISTS fr_velocity_count_week;
DROP TABLE IF EXISTS fr_velocity_count_delta;

SELECT date,
       sum(count) as count
  INTO fr_velocity_count_week
  FROM fr_tall_backlog
 WHERE status = '"resolved"'
   AND EXTRACT(dow from date) = 0 
   AND date >= current_date - interval '3 months'
...
SELECT date,
       ...
       NULL::int as velocity
  INTO fr_velocity_count_delta
  FROM fr_velocity_count_week
 ORDER BY date;

UPDATE fr_velocity_count_delta a
...
SELECT date,
       velocity
  FROM fr_velocity_count_delta
) TO '/tmp/fr_velocity_count.csv' DELIMITER ',' CSV HEADER;

/* ####################################################################
Backlog growth calculations */

DROP TABLE IF EXISTS fr_backlog_size;

SELECT date,
       SUM(points) AS points
  INTO fr_backlog_size
  FROM fr_tall_backlog
 WHERE status != '"resolved"'
   AND EXTRACT(dow from date) = 0 
 GROUP BY date
...
DROP TABLE IF EXISTS fr_maintenance_week;
DROP TABLE IF EXISTS fr_maintenance_delta;

SELECT date,
       maint_type,
       SUM(points) as points
  INTO fr_maintenance_week
  FROM fr_tall_backlog
  WHERE status = '"resolved"'
   AND EXTRACT(dow FROM date) = 0
   AND date >= current_date - interval '3 months'
...
SELECT date,
  ...
  INTO fr_maintenance_delta
  FROM fr_maintenance_week
 WHERE maint_type='Maintenance'
 ORDER BY date, maint_type;

UPDATE fr_maintenance_delta a
...
SELECT date,
  ...
  FROM fr_maintenance_delta
  ) as fr_maintenance_fraction
) TO '/tmp/fr_maintenance_fraction.csv' DELIMITER ',' CSV HEADER;
...
SELECT ROUND(100 * maint_points::decimal / nullif((maint_points + new_points),0),0) as "Total Maintenance Fraction"
  FROM (SELECT sum(maint_points) as maint_points
          FROM fr_maintenance_delta) as x
 CROSS JOIN 
       (SELECT sum(new_points)  as new_points
	  FROM fr_maintenance_delta) as y
) TO '/tmp/fr_maintenance_fraction_total.csv' DELIMITER ',' CSV;

/* by count */

DROP TABLE IF EXISTS fr_maintenance_count_week;
DROP TABLE IF EXISTS fr_maintenance_count_delta;

SELECT date,
       maint_type,
       count(date) as count
  INTO fr_maintenance_count_week
  FROM fr_tall_backlog
  WHERE status = '"resolved"'
   AND EXTRACT(dow FROM date) = 0
   AND date >= current_date - interval '3 months'
...
SELECT date,
  ...
  INTO fr_maintenance_count_delta
  FROM fr_maintenance_count_week
 WHERE maint_type='Maintenance'
 ORDER BY date, maint_type;

UPDATE fr_maintenance_count_delta a
...
SELECT date,
  ...
  FROM fr_maintenance_count_delta
  ) as fr_maintenance_count_fraction
) TO '/tmp/fr_maintenance_count_fraction.csv' DELIMITER ',' CSV HEADER;
...
SELECT ROUND(100 * maint_count::decimal / nullif((maint_count + new_count),0),0) as "Total Maintenance Fraction"
  FROM (SELECT sum(maint_count) as maint_count
          FROM fr_maintenance_count_delta) as x
 CROSS JOIN 
       (SELECT sum(new_count)  as new_count
	  FROM fr_maintenance_count_delta) as y
) TO '/tmp/fr_maintenance_count_fraction_total.csv' DELIMITER ',' CSV;
...
SELECT SUM(velocity)/3 AS min_velocity
  FROM (SELECT velocity 
         ...
         ORDER BY velocity 
         LIMIT 3) as x)
TO '/tmp/fr_min.csv' DELIMITER ',' CSV;
...
SELECT SUM(velocity)/3 AS max_velocity
  FROM (SELECT velocity 
         ...
         LIMIT 3) as x)
TO '/tmp/fr_max.csv' DELIMITER ',' CSV;
...
SELECT AVG(velocity) AS avg_velocity
  FROM (SELECT velocity 
         ...
         ORDER BY velocity)
         as x)
TO '/tmp/fr_avg.csv' DELIMITER ',' CSV;
...
SELECT category,
 ...
 GROUP BY category
 ORDER BY category)
 TO '/tmp/fr_backlog_current.csv' DELIMITER ',' CSV HEADER;
...
SELECT MAX(date)
  FROM fr_task_history)
TO '/tmp/fr_max_date.csv' DELIMITER ',' CSV;

/* ####################################################################
Recently Closed */

DROP TABLE IF EXISTS fr_recently_closed;

CREATE TABLE fr_recently_closed (
    date date,
    category text,
    points int,
    count int
...
SELECT * FROM fr_find_recently_closed();

COPY (
SELECT date,
       category,
       points
  FROM fr_recently_closed
 ORDER BY date, category
) to '/tmp/fr_recently_closed.csv' DELIMITER ',' CSV HEADER;
