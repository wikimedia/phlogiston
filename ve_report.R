...
backlog$category2 <- factor(backlog$category, levels = c("Uncategorized", "TR5: Rich media tools", "TR4: Link editor tweaks", "TR3: Language support", "TR2: Mobile MVP", "TR1: Releases", "VisualEditor 2014/15 Q4 blockers", "VisualEditor 2014/15 Q3 blockers", "TR0: Interrupt", "Miscategorized"))
...
ggplot(backlog_zoomed) +
  labs(title="VE Planned backlog (excludes Maintenance and Uncategorized)", y="Story Point Total") +
  theme(text = element_text(size=30), legend.title=element_blank()) +
  geom_area(position='stack', aes(x = date, y = points, group=category2, fill=category2, order=-as.numeric(category2))) +
...
done$category2 <- factor(done$category, levels = c('Other', 'Charts, formulae, sheet music, and media available or improved in VE', 'Language support defaults on for ja, ko, ar, fa, hi, zh_yue', 'Link Editor is better', 'Mobile MVP released', 'VE defaults on for wp_en new users and IPs', 'VE is more stable and A/B tested', 'VE is usable and has acceptable performance' ))
...
##   labs(title="Age of VE backlog (open tasks), excluding Uncategorized", y="Story Point Total") +
##   theme(text = element_text(size=30)) +
##   geom_bar(stat="identity") +
