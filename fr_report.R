...
backlog <- read.csv("/tmp/fr_backlog.csv")
backlog$date <- as.Date(backlog$date, "%Y-%m-%d")
backlog_output=png(filename = "~/html/fr_backlog_burnup.png", width=2000, height=1125, units="px", pointsize=30)

burnup <- read.csv("/tmp/fr_burnup.csv")
burnup$date <- as.Date(burnup$date, "%Y-%m-%d")

ggplot(backlog) +
  labs(title="Fundraising Tech backlog", y="Story Point Total") +
...
backlog_count <- read.csv("/tmp/fr_backlog_count.csv")
backlog_count$date <- as.Date(backlog_count$date, "%Y-%m-%d")
backlog_count_output=png(filename = "~/html/fr_backlog_count_burnup.png", width=2000, height=1125, units="px", pointsize=30)

burnup_count <- read.csv("/tmp/fr_burnup_count.csv")
burnup_count$date <- as.Date(burnup_count$date, "%Y-%m-%d")

ggplot(backlog_count) +
  labs(title="Fundraising Tech backlog", y="Task Count") +
  theme(text = element_text(size=30), legend.title=element_blank())+
...
## fr_maint_frac <- read.csv("/tmp/fr_maintenance_fraction.csv")
## fr_maint_frac$date <- as.Date(fr_maint_frac$date, "%Y-%m-%d")

## status_output <- png(filename = "~/html/fr_maint_frac.png", width=2000, height=1125, units="px", pointsize=30)
  
## ggplot(fr_maint_frac, aes(date, maint_frac)) +
...
## fr_maint_count_frac <- read.csv("/tmp/fr_maintenance_count_fraction.csv")
## fr_maint_count_frac$date <- as.Date(fr_maint_count_frac$date, "%Y-%m-%d")

## status_output_count <- png(filename = "~/html/fr_maint_count_frac.png", width=2000, height=1125, units="px", pointsize=30)
  
## ggplot(fr_maint_count_frac, aes(date, maint_frac)) +
##   labs(title="Fundraising Tech Maintenance Fraction (by count instead of points)", y="Fraction of completed work that is maintenance") +
##   geom_bar(stat="identity") +
##   theme(text = element_text(size=30)) +
##   scale_y_continuous(labels=percent, limits=c(0,1))
## dev.off()
...
velocity <- read.csv("/tmp/fr_velocity.csv")
velocity$date <- as.Date(velocity$date, "%Y-%m-%d")

velocity_output <- png(filename = "~/html/fr_velocity.png", width=2000, height=1125, units="px", pointsize=30)
...
velocity_count <- read.csv("/tmp/fr_velocity_count.csv")
velocity_count$date <- as.Date(velocity_count$date, "%Y-%m-%d")

velocity_count_output <- png(filename = "~/html/fr_velocity_count.png", width=2000, height=1125, units="px", pointsize=30)
...
net_growth <- read.csv("/tmp/fr_net_growth.csv")
net_growth$date <- as.Date(net_growth$date, "%Y-%m-%d")

net_growth_output <- png(filename = "~/html/fr_net_growth.png", width=2000, height=1125, units="px", pointsize=30)
...
forecast_output <- png(filename = "~/html/fr_forecast.png", width=2000, height=2000, units="px", pointsize=30)

ggplot(forecast, aes(x=foredate, y=date, group=case)) +
  geom_line(shape=1) +
  labs(title="Fundraising Tech Forecasting History", x="Now", y="Forecast Completion") +
...
done <- read.csv("/tmp/fr_recently_closed.csv")
done$date <- as.Date(done$date, "%Y-%m-%d")

done_output <- png(filename = "~/html/fr_done.png", width=2000, height=1125, units="px", pointsize=30)
ggplot(done, aes(x=date, y=points, fill=factor(category), order=-as.numeric(category))) +
  labs(title="Fundraising Tech Completed work", y="Points", x="Month", aesthetic='Milestone') +
  theme(text = element_text(size=30)) +
