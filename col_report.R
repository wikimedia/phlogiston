...
forecast_output <- png(filename = "~/html/col_forecast.png", width=2000, height=2000, units="px", pointsize=30)
...
## leadtime <- read.csv("/tmp/col_leadtime.csv")
## leadtime$week <- as.Date(leadtime$week, "%Y-%m-%d")
## leadtime_output <- png(filename = "~/html/col_leadtime.png", width=2000, height=1125, units="px", pointsize=30)
## ggplot(leadtime, aes(x=week, y=points, fill=leadtime)) +
##   labs(title="VE Lead Time for resolved tasks", y="Story Point Total") +
##   geom_bar(stat="identity")
## dev.off()

## age_of_resolved <- read.csv("/tmp/col_age_of_resolved.csv")
## age_of_resolved$week <- as.Date(age_of_resolved$week, "%Y-%m-%d")
## age_of_resolved_output <- png(filename = "~/html/col_age_of_resolved.png", width=2000, height=1125, units="px", pointsize=30)
...
## age_of_resolved_count <- read.csv("/tmp/col_age_of_resolved_count.csv")
## age_of_resolved_count$week <- as.Date(age_of_resolved_count$week, "%Y-%m-%d")
## age_of_resolved_count_output <- png(filename = "~/html/col_age_of_resolved_count.png", width=2000, height=1125, units="px", pointsize=30)
...
## backlogage <- read.csv("/tmp/col_age_of_backlog_specific.csv")
## backlogage$week <- as.Date(backlogage$week, "%Y-%m-%d")
## backlogage_output <- png(filename = "~/html/col_age_of_backlog_specific.png", width=2000, height=1125, units="px", pointsize=30)
