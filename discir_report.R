...
backlog <- read.csv("/tmp/discir_backlog.csv")
backlog$date <- as.Date(backlog$date, "%Y-%m-%d")
backlog_output=png(filename = "~/html/discir_backlog_burnup.png", width=2000, height=1125, units="px", pointsize=30)

burnup <- read.csv("/tmp/discir_burnup.csv")
burnup$date <- as.Date(burnup$date, "%Y-%m-%d")

ggplot(backlog) +
  labs(title="Discovery Cirrus backlog", y="Story Point Total") +
...
backlog_count <- read.csv("/tmp/discir_backlog_count.csv")
backlog_count$date <- as.Date(backlog_count$date, "%Y-%m-%d")
backlog_count_output=png(filename = "~/html/discir_backlog_count_burnup.png", width=2000, height=1125, units="px", pointsize=30)

burnup_count <- read.csv("/tmp/discir_burnup_count.csv")
burnup_count$date <- as.Date(burnup_count$date, "%Y-%m-%d")

ggplot(backlog_count) +
  labs(title="Discovery Cirrus backlog", y="Task Count") +
  theme(text = element_text(size=30), legend.title=element_blank())+
...
velocity <- read.csv("/tmp/discir_velocity.csv")
velocity$date <- as.Date(velocity$date, "%Y-%m-%d")

velocity_output <- png(filename = "~/html/discir_velocity.png", width=2000, height=1125, units="px", pointsize=30)
...
velocity_count <- read.csv("/tmp/discir_velocity_count.csv")
velocity_count$date <- as.Date(velocity_count$date, "%Y-%m-%d")

velocity_count_output <- png(filename = "~/html/discir_velocity_count.png", width=2000, height=1125, units="px", pointsize=30)
...
net_growth <- read.csv("/tmp/discir_net_growth.csv")
net_growth$date <- as.Date(net_growth$date, "%Y-%m-%d")

net_growth_output <- png(filename = "~/html/discir_net_growth.png", width=2000, height=1125, units="px", pointsize=30)
...
forecast_output <- png(filename = "~/html/discir_forecast.png", width=2000, height=2000, units="px", pointsize=30)

ggplot(forecast, aes(x=foredate, y=date, group=case)) +
  geom_line(shape=1) +
  labs(title="Discovery Cirrus Forecasting History", x="Now", y="Forecast Completion") +
...
done <- read.csv("/tmp/discir_recently_closed.csv")
done$date <- as.Date(done$date, "%Y-%m-%d")

done_output <- png(filename = "~/html/discir_done.png", width=2000, height=1125, units="px", pointsize=30)
ggplot(done, aes(x=date, y=points, fill=factor(category), order=-as.numeric(category))) +
  labs(title="Discovery Cirrus Completed work", y="Points", x="Month", aesthetic='Milestone') +
  theme(text = element_text(size=30)) +
