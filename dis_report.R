...
ggplot(backlog) +
  labs(title="Discovery Analysis backlog", y="Story Point Total") +
...
ggplot(backlog_count) +
  labs(title="Discovery Analysis backlog", y="Task Count") +
  theme(text = element_text(size=30), legend.title=element_blank())+
...
##   labs(title="Discovery Analysis Maintenance Fraction (by count instead of points)", y="Fraction of completed work that is maintenance") +
##   geom_bar(stat="identity") +
##   theme(text = element_text(size=30)) +
##   scale_y_continuous(labels=percent, limits=c(0,1))
## dev.off()
...
ggplot(forecast, aes(x=foredate, y=date, group=case)) +
  geom_line(shape=1) +
  labs(title="Discovery Analysis Forecasting History", x="Now", y="Forecast Completion") +
...
ggplot(done, aes(x=date, y=points, fill=factor(category), order=-as.numeric(category))) +
  labs(title="Discovery Analysis Completed work", y="Points", x="Month", aesthetic='Milestone') +
  theme(text = element_text(size=30)) +
