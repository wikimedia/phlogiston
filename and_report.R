...
backlog_output=png(filename = "~/html/and_backlog_burnup.png", width=2000, height=1125, units="px", pointsize=30)
...
backlog_count <- read.csv("/tmp/and_backlog_count.csv")
backlog_count$date <- as.Date(backlog_count$date, "%Y-%m-%d")
backlog_count_output=png(filename = "~/html/and_backlog_count_burnup.png", width=2000, height=1125, units="px", pointsize=30)

burnup_count <- read.csv("/tmp/and_burnup_count.csv")
...
## and_maint_frac <- read.csv("/tmp/and_maintenance_fraction.csv")
## and_maint_frac$date <- as.Date(and_maint_frac$date, "%Y-%m-%d")

## status_output <- png(filename = "~/html/and_maint_frac.png", width=2000, height=1125, units="px", pointsize=30)
  
## ggplot(and_maint_frac, aes(date, maint_frac)) +
...
## and_maint_count_frac <- read.csv("/tmp/and_maintenance_count_fraction.csv")
## and_maint_count_frac$date <- as.Date(and_maint_count_frac$date, "%Y-%m-%d")

## status_output_count <- png(filename = "~/html/and_maint_count_frac.png", width=2000, height=1125, units="px", pointsize=30)
  
## ggplot(and_maint_count_frac, aes(date, maint_frac)) +
##   labs(title="Android Maintenance Fraction (by count instead of points)", y="Fraction of completed work that is maintenance") +
##   geom_bar(stat="identity") +
##   theme(text = element_text(size=30)) +
##   scale_y_continuous(labels=percent, limits=c(0,1))
## dev.off()
...
velocity_output <- png(filename = "~/html/and_velocity.png", width=2000, height=1125, units="px", pointsize=30)
...
velocity_count <- read.csv("/tmp/and_velocity_count.csv")
velocity_count$date <- as.Date(velocity_count$date, "%Y-%m-%d")

velocity_count_output <- png(filename = "~/html/and_velocity_count.png", width=2000, height=1125, units="px", pointsize=30)
...
net_growth <- read.csv("/tmp/and_net_growth.csv")
net_growth$date <- as.Date(net_growth$date, "%Y-%m-%d")

net_growth_output <- png(filename = "~/html/and_net_growth.png", width=2000, height=1125, units="px", pointsize=30)
...
forecast_output <- png(filename = "~/html/and_forecast.png", width=2000, height=2000, units="px", pointsize=30)

ggplot(forecast, aes(x=foredate, y=date, group=case)) +
  geom_line(shape=1) +
  labs(title="Android Forecasting History", x="Now", y="Forecast Completion") +
...
done <- read.csv("/tmp/and_recently_closed.csv")
done$date <- as.Date(done$date, "%Y-%m-%d")

done_output <- png(filename = "~/html/and_done.png", width=2000, height=1125, units="px", pointsize=30)
ggplot(done, aes(x=date, y=points, fill=factor(category), order=-as.numeric(category))) +
  labs(title="Android Completed work", y="Points", x="Month", aesthetic='Milestone') +
  theme(text = element_text(size=30)) +
